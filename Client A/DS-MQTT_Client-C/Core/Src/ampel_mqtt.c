...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    if (status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        mqtt_subscribe(client, "Ampel/A/#", 0, mqtt_sub_request_cb, NULL);
    }
    else
    {
        mqtt_connected = 0;
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags)
{
    ...
    if (strcmp(current_topic, "Ampel/A/Fuss") == 0)
    {
    ...
    if (strcmp(current_topic, "Ampel/A/Haupt") == 0)
    {
        if (strncmp((const char *)data, "red", 3) == 0)
        {
    ...
    if (strcmp(current_topic, "Ampel/A/Abbiege") == 0)
    {
